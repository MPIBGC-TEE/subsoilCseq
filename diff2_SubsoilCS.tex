...
%DIF DEL subsoilCS_R1.tex   Fri Nov  3 17:12:26 2023
%DIF ADD subsoilCS_R2.tex   Wed Dec 20 12:02:48 2023
...
%DIF 25-26c25-26
%DIF < \author{Maarten C. Braakhekke \orcidlink{0000-0002-1889-2225}}
%DIF < \author[1,3]{Sophie von Fromm \orcidlink{https://orcid.org/0000-0002-1820-1455}}
%DIF -------
\author[3]{Maarten C. Braakhekke \orcidlink{0000-0002-1889-2225}} %DIF > 
\author[1,4]{Sophie von Fromm \orcidlink{https://orcid.org/0000-0002-1820-1455}} %DIF > 
%DIF -------
\author[2]{Thomas K\"atterer \orcidlink{0000-0002-1751-007X}}
%DIF 28c28
%DIF < \author[4]{Zhongkui Luo \orcidlink{0000-0002-6744-6491}}
%DIF -------
\author[5]{Zhongkui Luo \orcidlink{0000-0002-6744-6491}} %DIF > 
%DIF -------
\author[2]{Nargish Parvin \orcidlink{0000-0002-1571-575X}}
%DIF 30c30
%DIF < \author[5]{Guocheng Wang \orcidlink{0009-0006-3492-7105}}
%DIF -------
\author[6]{Guocheng Wang \orcidlink{0009-0006-3492-7105}} %DIF > 
%DIF -------

%DIF 32-36c32-37
%DIF < \affil[1]{Max Planck Institute for Biogeochemistry, Germany}
%DIF < \affil[2]{Department of Ecology, Swedish University of Agricultural Sciences, Sweden}
%DIF < \affil[3]{Department of Environmental Science, ETH Zurich, Switzerland}
%DIF < \affil[4]{College of Environmental and Resource Sciences, Zhejiang University, Hangzhou, China}
%DIF < \affil[5]{Faculty of Geographical Science, Beijing Normal University, Beijing, China} %, 100875}
%DIF -------
\affil[1]{Max Planck Institute for Biogeochemistry, Jena, Germany} %DIF > 
\affil[2]{Department of Ecology, Swedish University of Agricultural Sciences, Uppsala, Sweden} %DIF > 
\affil[3]{Wageningen Environmental Research, Wageningen UR, Wageningen, the Netherlands} %DIF > 
\affil[4]{Department of Environmental Science, ETH Zurich, Switzerland} %DIF > 
\affil[5]{College of Environmental and Resource Sciences, Zhejiang University, Hangzhou, China} %DIF > 
\affil[6]{Faculty of Geographical Science, Beijing Normal University, Beijing, China} %, 100875} %DIF > 
%DIF -------
...
\DIFdelbegin \DIFdel{Land management activities could increase C storage in the subsoil by increasing advective vertical transport and decreasing the difference between inputs and decomposition at all depths. 
}\DIFdelend %DIF > Land management activities could increase C storage in the subsoil by increasing advective vertical transport and decreasing the difference between inputs and decomposition at all depths. 
\end{abstract}
...
Models explicitly representing decomposition usually use one or three pools to represent decomposition as in most traditional models. More detailed representations of decomposition are presented in the model of \citet{Braakhekke2011, Braakhekke2013}, which represents five different pools, including a litter layer component clearly separating processes related to decomposition in the surface organic layer from processes more affected by vertical transport in the mineral horizons. Also, the model of \citet{Koven2013BGS} used 7 distinct C pools: coarse woody debris, three litter pools, and \DIFdelbegin \DIFdel{3 }\DIFdelend \DIFaddbegin \DIFadd{three }\DIFaddend mineral soil C pools. 
...
\end{equation}
with the maximum decomposition rate at the surface given by $k_0 = k(d=0)$, and $d_e$ representing the e-folding depth of decomposition rates. In our simulations, we used values of $k_0 : \{1, 0.1, 0.01 \}$ yr$^{-1}$ and a constant value of $d_e = 90$ cm\DIFdelbegin \DIFdel{(the model showed little sensitivity to different values of $d_e$)}\DIFdelend . Equation \ref{eq:kd} is an empirical function that accounts for unresolved processes such as changes in oxygen availability or microbial activity with depth, and therefore it could be considered as a place holder for other mechanistic representations of depth dependent microbial dynamics. 

The results from this second set of simulations evaluating the effect of lateral root inputs and decomposition showed that slowing down decomposition can have a significant effect on the shape of the vertical soil C profile (Figure \ref{fig:rootDecomp}). These results seem counterintuitive because equation \ref{eq:simpleCase} \DIFdelbegin \DIFdel{suggest }\DIFdelend \DIFaddbegin \DIFadd{suggests }\DIFaddend that the negative term of the equation should be affected by larger values of $k(d)$, but because with slow decomposition higher amounts of C are obtained at steady-state, the entire term $k(d) x(d)$ is large, promoting a strong soil C gradient. The vertical distribution of root inputs has also a significant effect on the shape of the soil C profile, with shallow root inputs promoting a strong vertical gradient and deep rooting systems a more pronounced gradient with lower values of the first derivative (Figure \ref{fig:rootDecomp}). In this set of simulations, we observed also a maximum rate of change of C at the upper layers where the value of the first derivative reached a maximum. 
...
For a compartmental system in equilibrium, where carbon inputs are balanced with C losses, the fate of C entering at a time $t_0$ can be obtained as a function that predicts the mass of C remaining in the soil at \DIFdelbegin \DIFdel{any given time $t > t_0$ \mbox{%DIFAUXCMD
\citep{Sierra2021JE}
}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{time $t$, thus we define $\tau = t- t_0$, and  
}\DIFaddend \begin{equation} \DIFaddbegin \label{eq:m}
\DIFaddend \bm{m}(\DIFdelbegin \DIFdel{t}\DIFdelend \DIFaddbegin \DIFadd{\tau}\DIFaddend ) = e^{\tau \mathbf{B}} \ \bm{u},
\end{equation}
\DIFdelbegin \DIFdel{where $\bm{m}(t)$ }\DIFdelend \DIFaddbegin \DIFadd{as in \mbox{%DIFAUXCMD
\citet{Sierra2021JE}}\hskip0pt%DIFAUXCMD
, where $\bm{m}(\tau)$ }\DIFaddend is a vector with the mass remaining for each compartment. This mass remaining is related to the transit time of carbon, which is defined as the time it takes carbon atoms to pass through the entire network of compartments until C leaves the soil system \citep{Bolin1973, Manzoni2009JGR, Sierra2018JAMES}. The transit time distribution of carbon can be expressed as \citep{Metzler2018MG}
...
Mean transit time (yr\DIFdelbeginFL \DIFdelFL{$^{-1}$}\DIFdelendFL ) & 1.333  & 9.699 & 1.217 & 11.498 \\
Median transit time (yr\DIFdelbeginFL \DIFdelFL{$^{-1}$}\DIFdelendFL ) & 0.859  & 9.444 & 0.799  & 7.096 \\
...
\begin{figure}[htbp]
   \centering
   \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Figures/ISRaD_msp_SOC_climate_depth_profile_deriv_2023-04-11.jpeg} %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Figures/profiles.pdf} \DIFaddendFL % requires the graphicx package
   \caption{Soil carbon concentrations \DIFdelbeginFL \DIFdelFL{at different depths }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with 1st and 2nd derivatives with respect to depth obtained }\DIFaddendFL from the International Soil Radiocarbon Database, with data from 600 profiles aggregated by biogeographical regions. Thick lines for each group represent the mean across available observations and fitted to a spline curve. Horizontal lines represent standard deviation across available observations. }
   \label{fig:Cprofiles}
\end{figure}
...
%DIFDELCMD <    \centering
%DIFDELCMD <    \includegraphics[width=\textwidth]{Figures/ISRaD_msp_14C_SOC_climate_depth_deriv_2023-03-13.jpeg} %%%
%DIF <  requires the graphicx package
   %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{First and second derivative of soil C profiles extracted from ISRaD and grouped by biogeographical region, with the exception of volcanic soils.}}
   %DIFAUXCMD
%DIFDELCMD < \label{fig:Derivatives}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF > \begin{figure}[htbp]
%DIF >    \centering
%DIF >    \includegraphics[width=\textwidth]{Figures/ISRaD_msp_14C_SOC_climate_depth_deriv_2023-03-13.jpeg} % requires the graphicx package
%DIF >    \caption{First and second derivative of soil C profiles extracted from ISRaD and grouped by biogeographical region, with the exception of volcanic soils.}
%DIF >    \label{fig:Derivatives}
%DIF > \end{figure}

The first derivative of soil C concentrations with respect to depth ($\partial x(d)/ \partial d$, Figure \DIFdelbegin \DIFdel{\ref{fig:Derivatives}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:Cprofiles}}\DIFaddend )  was negative for all groups, indicating that soil C always decreases with depth for these aggregated profiles. This is in  agreement with our previous simulations in which the first derivatives were always negative. We observed for all groups a peak in the first derivative where it reaches a maximum negative value, indicating that soil C decreases more strongly at some intermediate depth between 10 and 20 cm (Figure \DIFdelbegin \DIFdel{\ref{fig:Derivatives}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:Cprofiles}}\DIFaddend ). According to equation \ref{eq:simpleCase}, a maximum negative value of the first derivative can only occur at depths where the microbial decomposition flux ($k(d) x(d)$) has its maximum value, i.e. when microbes are consuming the maximum amount of carbon possible.
...
Overall, the data from soil C profiles aggregated by regional groups and volcanic soils provide evidence supporting the idea that vertical transport may play a secondary role in determining the rate of soil C decrease with depth. The difference between lateral root inputs and decomposition may play a primary role in determining the shape of soil C profiles, and this difference may be amplified at low advection velocity rates. Diffusive movement of soil C seems to play a small role in these aggregated groups, something suggested by the low values of the second derivative (Figure \DIFdelbegin \DIFdel{\ref{fig:Derivatives}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:Cprofiles}}\DIFaddend ); however, diffusion may have some control on the peak of C decrease found close to the surface in these profiles. 
...
\begin{figure}[htbp]
   \centering
   \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Figures/biomeTransitTimes.pdf} %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.65]{Figures/biomeTT.pdf} \DIFaddendFL % requires the graphicx package
   \caption{Estimates of mean age and mean transit time of carbon based on measurements of root inputs and soil radiocarbon obtained from ISRaD. The left panel shows global-scale average values of mean age and mean transit time. The \DIFdelbeginFL \DIFdelFL{right }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lower }\DIFaddendFL panel shows averages of mean transit time \DIFdelbeginFL \DIFdelFL{averaged }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{aggregated }\DIFaddendFL by biome.}
